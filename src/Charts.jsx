...
class Charts extends Component {
    ...
    componentDidMount() {
        ...
        if (window.innerWidth < 800)
            this.setState({ screen_size: null })
    }
    ...
    getInvestment = () => {
        ...
        this.props.chartDate.forEach((month, index) => {
            ...
            invested.push({ y: investment - month.month.savings, label: `Mes ${index}` })
            saved.push({ y: month.month.savings, label: `Mes ${index}` })
            total.push({ y: investment + month.month.savings, label: `Mes ${index}` })
        });
